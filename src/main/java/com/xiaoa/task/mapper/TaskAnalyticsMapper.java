...
public interface TaskAnalyticsMapper {
    ...
    @Select("SELECT tr.store_id AS storeId, s.name AS storeName, COUNT(*) AS expected, "
            + "SUM(CASE WHEN tr.status = 1 THEN 1 ELSE 0 END) AS finished FROM task_record tr "
            ...
            + "AND (#{regionId} IS NULL OR s.parent_id = #{regionId}) GROUP BY tr.store_id, s.name ORDER BY tr.store_id")
    ...
    @Select("SELECT tr.id, tr.task_id AS taskId, tr.user_id AS userId, u.nickname, tr.store_id AS storeId, "
            ...
            + "p.proof_url AS proofUrl, tr.finished_at AS finishedAt FROM task_record tr "
            + "JOIN `user` u ON u.id = tr.user_id JOIN `org` s ON s.id = tr.store_id AND s.tenant_id = tr.tenant_id "
    ...
    @Select("SELECT r.user_id AS userId, MAX(u.nickname) AS nickname, r.org_id AS storeId, MAX(s.name) AS storeName, "
            + "COUNT(*) AS expected, SUM(CASE WHEN tr.status = 1 THEN 1 ELSE 0 END) AS finished, "
            + "MAX(tr.finished_at) AS lastFinishedAt FROM task_record tr "
            + "JOIN user_org_role r ON r.tenant_id = tr.tenant_id AND r.user_id = tr.user_id "
            + "AND r.org_id = tr.store_id AND r.status = 1 AND r.role IN ('STAFF', 'OWNER') "
            + "JOIN `user` u ON u.id = r.user_id AND u.deleted = 0 JOIN `org` s ON s.id = r.org_id "
            + "AND s.tenant_id = r.tenant_id WHERE tr.tenant_id = #{tenantId} "
            + "AND tr.period_date >= #{from} AND tr.period_date < #{to} "
            + "AND (#{storeId} IS NULL OR tr.store_id = #{storeId}) "
            + "AND (#{regionId} IS NULL OR s.parent_id = #{regionId}) GROUP BY r.user_id, r.org_id "
            + "ORDER BY finished DESC, (finished / NULLIF(COUNT(*), 0)) DESC, lastFinishedAt ASC LIMIT #{limit}")
